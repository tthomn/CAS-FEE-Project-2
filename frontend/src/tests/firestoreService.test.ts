...
describe('firestoreService', () => {
  ...
  describe('getDocRefsBy1Condition', () => {
    it('should return document references matching one condition', async () => {
      ...
      (getDocs as jest.Mock).mockResolvedValueOnce(
        createQuerySnapshot(docsWithRefs),
      );

      const result = await getDocRefsBy1Condition(
        'orders',
        'status',
        '==',
        'open',
  ...
  describe('getCollectionData', () => {
    it('fetches entire collection data (no constraints)', async () => {
      const data = await getCollectionData<{ name: string; age: number }>(
        'users',
      );
      expect(collection).toHaveBeenCalledWith(expect.anything(), 'users');
    ...
    it('throws error if fetching fails', async () => {
      (getDocs as jest.Mock).mockRejectedValueOnce(
        new Error('Collection fetch error'),
      );
      await expect(getCollectionData('users')).rejects.toThrow(
  ...
  describe('addDocToCollection', () => {
    ...
    it('throws if adding fails', async () => {
      ...
      await expect(addDocToCollection('users', { foo: 'bar' })).rejects.toThrow(
        'Add doc error',
      );
    });
  });
  ...
  describe('updateDocByRef', () => {
    ...
    it('throws if update fails', async () => {
      (updateDoc as jest.Mock).mockRejectedValueOnce(
        new Error('Update doc error'),
      );
      await expect(
        updateDocByRef(mockDocRef, { name: 'Fail' }),
      ).rejects.toThrow('Update doc error');
    });
  });
  ...
  describe('deleteDocByRef', () => {
    ...
    it('throws if delete fails', async () => {
      (deleteDoc as jest.Mock).mockRejectedValueOnce(
        new Error('Delete doc error'),
      );
      await expect(deleteDocByRef(mockDocRef)).rejects.toThrow(
        'Delete doc error',
      );
    });
  });
